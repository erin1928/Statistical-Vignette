...
- Learn the Anderson Darling Equation and understand how it can be used in R.  
...
The Anderson Darling test is a form of the Goodness of Fit test used to determine if a sample of data comes from a specific kind of distribution, commonly normal distribution. It is a modification of the Kolmogorov-Smirnov test and focuses more on whether or not the tails follow normal distribution, rather than the peak. 
...
The results will suggest that you can either reject the null, or fail to reject the null. The test rejects the null hypothesis of normality when the p-value is **less than or equal to 0.05** and accepts the null hypothesis (fails to reject the null) if it is **greater than 0.05.** 

## Anderson Darling Test Formula: 

The formula below gets you the test statistic for your specified distribution, which will be modified to find the p-value. The test statistic needs to be modified for small data sets. 
 
![](Anderson-formula.png)

- n = the sample size
- F(X) = CDF (cumulative distribution function) for the specified distribution
- i = the ith sample, calculated when the data is sorted in ascending order (1:n)

 Using the test statistic, you will then modify it using the following equation: Z = AD(1.0 + 0.75/n + 2.25/n^2)
...
#### Determining if the Weight Variable is Normally Distributed 
...
#### Determining the Test Statistic (A Value) Step by Step 
The set of equations below are derived from the formula depicted above and taken from ad.test() the built in R function we will use later in this module: 
```{r}
x <- sort(a[complete.cases(a)]) #a is the data set that you want to test, complete.cases() is for removing missing values from a vector, sort() puts the data in ascending order, which is necessary to compute the anderson darling test statistic 
x
```

```{r}
n <- length(x) # reminder that your sample data has to be greater than 7 
n #n is 71 
```

```{r}
logp1 <- pnorm((x - mean(x))/sd(x), log.p = TRUE) #F(Xi) is the cumulative distribution and pnorm() allows us to calculate that - the mean/standard deviation of each data point is subtracted from each data point and then log.p() is used 
logp2 <- pnorm(-(x - mean(x))/sd(x), log.p = TRUE) #using pnorm() to find the compared cumulative distribution which is the equation -F(Xn-i+1)

```

```{r}
h <- (2 * seq(1:n) - 1) * (logp1 + rev(logp2)) #summation part of the equation (seq 1:n makes sure to order your data in ascending order, rev() is used to reverse the formula we computed in logp2)
```
...
#### Determining the Modified Test Statistic  
```{r}
...
#### Determine the Test Statistic (A) 

```{r}
x <- sort(b[complete.cases(b)]) 
n <- length(x) 
...
#### Determine the Modified Test Statistic (AA) 
```{r}
AA <- (1 + 0.75/n + 2.25/n^2) * A
AA 
```
...
#### Find the p-value based on the criteria of the Modified Test Statistic (AA)
```{r}
pval <- exp(0.9177 - 4.279 * AA - 1.38 * AA^2)
pval
```
...
## Challenge 2
Create a data set using a poisson function that is not distributed normally. Use the ad.test() to confirm that the data is not normally distributed. Plot the data using a histogram.
...
```
The data set is normally distributed because the p-value >0.05
...
ad.test(rpois(100,5))
```
The data set is not normally distributed because the p-value <0.05 
