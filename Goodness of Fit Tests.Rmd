...
It is important to note that the length of the sample data  must be **greater than 7**, but is advised to be **greater than 20**. 
...
Once the test statistic is calculated, you will then modify it using the following equation: Z = AD(1.0 + 0.75/n + 2.25/n^2). From here, the p-value can be calculated based on an equation that is determined by the modified test statistic. Once we calculate the p-value, we can determine whether we can reject the null hypothesis or fail to reject the null.
...
```
It can be concluded that chick weight data follows normal distribution because the p-value is greater than 0.05. 
...
```
Because the p-value is greater than 0.05, it can be determined that the data follows normal distribution and that the null hypothesis has failed being rejected. 
...
```
The data set follows normal distribution because the p-value >0.05
...
ad.test(rpois(100,5))
```
The data set does not follow normal distribution because the p-value <0.05 
