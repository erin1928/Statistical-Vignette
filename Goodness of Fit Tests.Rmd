...
Before starting to work with a data set, you have to figure out what kind of data it is. Goodness of fit tests report if your data is normal or matches with expected data. Then you are able to pick the best statistical test to run on your data. Different data types have different goodness of fit tests and each test is used to draw different conclusions. Each goodness of fit test has a null and alternate hypothesis and the calculated p-value allows us to reject the null. For a refresher on hypothesis testing see [Module 10](https://fuzzyatelin.github.io/bioanth-stats/module-10/module-10.html).<br>


Previously in the course we learned to use the Chi-squared goodness of fit test in [Module 14](https://fuzzyatelin.github.io/bioanth-stats/module-14/module-14.html#Chi-Square_Tests_of_Goodness_of_Fit_and_Independence). Goodness of fit tests can be used to assess how well Generalized Linear Models fit the data [Module 16](https://fuzzyatelin.github.io/bioanth-stats/module-17/module-17.html).<br>
...
Alternate code via Lia Bao
```{r}
unique(d$Flower.species) # unique elements in the data.
n <- length(unique(d$Flower.species)) #number of elements
n
fs <- c(as.numeric(table(d$Flower.species))) #extract the numerical data from table
fs
fp <- c(rep(1/n, each=n)) # set equal proportions repeated
fp
GTest(x=fs, 
      p=fp)
```
This tells us that the species of each iris is equally distributed. 
...
d %>% group_by(Flower.color, Flower.size) %>%  # Group by independent variables
  dplyr::count(Flower.color, Flower.size) # count how many are in each group

obs <- rbind(c(64, 22, 327, 210), # Small flowers
             c(57, 37, 301, 182)) # Large flowers
...
ks.test(data1, data2)
```
Let's take a look at this value graphically! To do this we are going to use our data1 and data2 sets we created, and then graph them using an ECDF. An ECDF (Empirical distribution function) creates a cumulative distribution function for our observed data, which we are able to graph together and view the difference in. This code was originally created by Matt Harris (you can check out his site <a href=https://rpubs.com/mharris/KSplot>here</a>)
```{r test, include = TRUE}
#group both of our data sets into one variable we can call later on.
group <- c(rep("data1", length(data1)), rep("data2", length(data2)))
#create a data frame to work with and call from.
dat <- data.frame(KSD = c(data1,data2), group = group)
# create ECDF of our dats in base are and calling it a memorable variable
ecdf1 <- ecdf(data1) 
ecdf2 <- ecdf(data2) 
# find minimum and maximum statistics! This will allow us to draw our line between our data sets
minMax <- seq(min(data1, data2), max(data1, data2), length.out=length(data1)) 
x0 <- minMax[which( abs(ecdf1(minMax) - ecdf2(minMax)) == max(abs(ecdf1(minMax) - ecdf2(minMax))) )] 
y0 <- ecdf1(x0) #setting points of reference
y1 <- ecdf2(x0) 

# loading in the required package
library(ggplot2)
ggplot(dat, aes(x = KSD, group = group, color = group))+ #calling our df, coloring our lines by groups
  stat_ecdf(size=1) +
    theme_bw(base_size = 30) + #adding a theme
    theme(legend.position ="top") +
    xlab("Data") + #naming our axis
    ylab("ECDF") +
    #geom_line(size=1) +
    ...
    ggtitle("K-S Test: data 1 / data 2") + #titling the graph
...
The Anderson Darling test is a type of the goodness of fit test used to determine if a sample of data follows a specific kind of distribution, more commonly normal distribution. It is a modification of the Kolmogorov-Smirnov test that focuses more on whether or not the tails follow normal distribution, rather than the peak. 
...
```
The p-value for each test is greater than 0.05 meaning that we can fail to reject the null hypothesis and that the data set does indeed follow normal distribution. This makes sense because it was created using the rnorm() function! Although both tests show that the data set follows normal distribution, the Shapiro Wilk test is more powerful than the Anderson Darling test and would be more accurate in this scenario.
