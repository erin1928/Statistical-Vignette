...
- Learn the Anderson Darling Equation and how it can be used in R.  
...
![](Anderson-formula.png)
 - AD is the formula for deriving the test statistic 
 - n is the sample size
 - F is the cumulative distribution function of the specified distribution. This gives you the cumulative (additive) probability associated with a function. 
 
 Using the test statistic, you will then modify it using the following equation: Z = A(1.0 + 0.75/n + 2.25/n^2)
...
#### Determining the Test Statistic (A Value) Step by Step Needed to Determine the Modified Test Statistic (AA Value) 
```{r}
x <- sort(a[complete.cases(a)]) #a is the data set that you want to test, complete.cases() is for removing missing values from a vector
