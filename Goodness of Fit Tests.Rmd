...
[K-S] Understand what a Kolmogorov-Smirnov Test is and how to interpret the results. 

[K-S] Learn what conditions are ideal for a K-S Test.

[A-D] Learn the Anderson Darling Equation and how it can be used in R.  

[A-D]Go through the Anderson Darling Test step-by-step and how to interpret the results. 

[A-D]Use the ad.test() built in function with Poisson functions. 

[S-W]Learn the Shapiro-Wilk Function and how it is derived.

[S-W]Use the shapiro.test() function in R and interpret the results. 
...
```
We got an output of 725.95 and the degree of freedom is options-1. We calculated a p-value of 4.9e-157 which is below the critical p-value = 0.05. We can reject the null hypothesis and conclude that the flower color proportions of the Irises observed are not equally distributed. 
...
Perform the G test with following columns in the iris data set: Flower.size, Flower.location and Flower.species.
...
The Anderson Darling Test starts with a null hypothesis that your data is not statistically different than normal. The alternate hypothesis is that it is different from normal. The results will suggest that you can either reject the null, or fail to reject the null. The test rejects the null hypothesis of normality when the p-value is **less than or equal to 0.05** and accepts the null hypothesis (fails to reject the null) if it is **greater than 0.05.** 

 - H0 is the data that follows a specific distribution (null hypothesis)
 - HA is the data that does not follow the specific distribution (alternative hypothesis)

## Anderson Darling Test Formula: 

The formula below gets you the test statistic for your specified distribution, which will be modified to find the p-value. 
 
A^2 = -N-S, where S= N∑i=1 ((2i−1)/N)(lnF(Yi)+ln(1−F(YN+1−i)) 
 
 - n is the sample size
 - F is the cumulative distribution function of the specified distribution. This gives you the cumulative (aditive) probability associated with a function. 
 - Yi is the ordered data. 


## Using the Anderson-Darling Test with Data Known as "chickwts" 

<font size="+2"> Newly hatched chicks were randomly allocated into six groups, and each group was given a different feed supplement. Their weights in grams after six weeks are given along with feed types. </font>
...
```{r}
x <- sort(a[complete.cases(a)]) #a is the data set that you want to test, complete.cases() is for removing missing values from q vector
...
#### Determining the AA Value from the A Value Needed to Determine the P Value  
```{r}
AA <- (1 + 0.75/n + 2.25/n^2) * A
AA #The AA value is computed (modified test statistic)
```

#### Determining the P Value Step by Step
"The p-value is derived from a modified statistic Z = A(1.0 + 0.75/n + 2.25/n^2) according to Table 4.9 in Stephens (1986)."
...
## Anderson Darling Test Challenge 1

Determine the A, AA, and P values of a sample of the chickwts data set. Is this sample of the data set originating from normal distribution? How do you know this?
...
```{r}
x <- sort(b[complete.cases(b)]) #b is the data set that you want to test, complete.cases() is for removing missing values from q vector
n <- length(x) #length must be greater than 7
logp1 <- pnorm((x - mean(x))/sd(x), log.p = TRUE)
logp2 <- pnorm(-(x - mean(x))/sd(x), log.p = TRUE)
h <- (2 * seq(1:n) - 1) * (logp1 + rev(logp2))
...
pval
```
The p-value is greater than 0.05, so the test fails to reject the null hypothesis meaning it originates from normal distribution. 
...
ndistribution <- rnorm(100, 25, 1) #The rnorm() function generates a random sample that is normally distributed
ndistribution
```
...
```
Because the p-value is greater than 0.05, it can be determined that the data is normally distributed and that the null hypothesis can be accepted. 

#### Plotting the Data 
```{r}
hist(ndistribution, main = "Histogram Showing Normal Distribution", col="blue")
```

<br> 

## Anderson Darling Challenge 2
Create a data set using a poisson function that is not distributed normally. Plot the data using a histogram and compare it to the normally distributed data. Use the ad.test() to confirm that the data is not normally distributed. 


```{r}
set.seed(1)
nodistribution <- rpois(100, 25) #rpois() can be used to create a sample that is not normally distributed
nodistribution
```
...
```
Because the p-value is less than or equal to 0.05, it can be determined that the data is different than normal and the null hypothesis is rejected.

#### Plotting the Data 
```{r}
hist(nodistribution, main= "Histogram Showing No Normal Distribution", col="red")
```
...
However, the Shapiro-Wilk Test can be really difficult to calculate, especially if you have more values, so there's a simple command in R to conduct this test! 
...
set.seed(1)
dataset2 <- rpois(35, lambda = 3) # making a poisson distributed data set
...
```
Both p-values are relatively close being only 0.1122 points away from one another. The Shapiro-Wilk test is best used for data sets smaller than 50, while the Anderson Darling test is best for data sets that are greater than seven, but are recommended to be greater than 20, as a reminder. 
...
The p-value appears to be far below 0.05, 0.0000000000000003331, which would mean that the null hypothesis is rejected and that this test deems the data set to be not normally distributed. This is an example as to why it is important to select the right goodness of fit test when analyzing data. 
