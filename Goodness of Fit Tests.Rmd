...
logp1 <- pnorm((x - mean(x))/sd(x), log.p = TRUE) #pnorm() provides the cumulative density of the normal distribution (of the equation F(Xi)
logp2 <- pnorm(-(x - mean(x))/sd(x), log.p = TRUE) #finding the cumulative density of the equation: -F(Xn-i+1)
h <- (2 * seq(1:n) - 1) * (logp1 + rev(logp2)) #second half of the equation 
A <- -n - mean(h) #first half of the equation
