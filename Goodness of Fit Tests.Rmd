...
ks.test(data1, data2)
```
Let's take a look at this value graphically! To do this we are going to use are data1 and data2 sets we created, and then graph them using an ECDF. An ECDF creates a cumulative distribution function for our observed data, which we are able to graph together and view the difference in. This code was originally created by Matt Harris (you can check out his site <a href=https://rpubs.com/mharris/KSplot>here</a>)
```{r test, include = TRUE}
...
ggplot(dat, aes(x = KSD, group = group, color = group))+
  stat_ecdf(size=1) +
    ...
    ggtitle("K-S Test: data 1 / data 2") +
