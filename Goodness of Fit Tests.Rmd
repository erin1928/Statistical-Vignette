...
```
Both p-values are relatively close being only 0.1122 points away from one another. Although the values are close, the Shapiro Wilk test is more powerful than the Anderson Darling test and would be more accurate in this scenario.

The p-value for each test is greater than 0.05 meaning that we failed to reject the null hypothesis and that the data set comes from normal distribution. This makes sense because it was created using the rnorm() function! 
